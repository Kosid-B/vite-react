...
🎢 **จังหวะอารมณ์ของหน้า Landing — `src/lib/emotionalArc.ts`** (เจ้าของสั่ง 24 ส.ค. 2569 · "ใช้หลัก hormone roller coaster")
⚠️ **ที่มาต้องพูดให้ตรง**: skill `ai-dark-marketing` ที่ sync มา **ไม่มีเนื้อหาเรื่องฮอร์โมนเลยสักบรรทัด**
   (grep หา dopamine/cortisol/oxytocin/serotonin = ไม่เจอ) ⇒ ไฟล์นี้เป็น **การออกแบบของเรา** ไม่ใช่การถอดจาก skill นั้น
🔴 **หลักการเดียวที่ทั้งไฟล์ยืนอยู่บน**: **ความตึงต้องมาก่อนความโล่ง และความโล่งต้องเป็นของจริง**
   หน้าที่ให้ความมั่นใจก่อนที่ผู้อ่านจะรู้สึกว่ามีปัญหา = ไม่มีอะไรให้คลาย ⇒ ไม่มีเหตุผลจะเลื่อนต่อ
   ความตึงของเรามาจาก **ตัวเลขของเขาเองที่เขาตอบไม่ได้** ไม่ใช่ความขาดแคลนปลอม (`FORBIDDEN_TRIGGERS` มี "ใช้อะไรแทน" ทุกข้อ)
📏 **ตัววัดคือ `flatnessDebt()` ไม่ใช่ "จำนวน warn"** (แก้ 26 ส.ค. 2569 · ledger #53)
   ตัวนับเดิม `floor(run/4)` ให้ **กองรวมกัน 10 อัน = 2 · กระจาย 4,4,4,5,5 = 5** ⇒ **ชี้ทางกลับด้าน**
   ⇒ ตัวใหม่คิดจาก **ส่วนที่ล้นเพดาน** `Σ max(0, run − 4)` · แปลตรง ๆ ว่า "ต้องเพิ่มจุดตึงอีกกี่จุด"
🧮 **สถานะจริง: หนี้ = 0** (26 ส.ค. 2569) — เดิม 8 → 4 (เรียงใหม่จนถึงพื้นทางคณิตศาสตร์) → **0** เมื่อเพิ่มจุดตึงจริงจุดที่ 7
   จุดตึงใหม่ = **ท้ายเครื่องประเมินตลาด** ปิดด้วยคำถาม *"แล้วปีที่ผ่านมา คุณได้ไปเท่าไหร่จากก้อนนี้"*
   (แทนคำสัญญาผลลัพธ์เดิมที่เราไม่มีหลักฐาน) ⇒ โล่งกลางหน้า 23 · จุดตึง 7 → ความจุ 6 ช่อง × 4 = 24
   🔒 เทสต์บังคับ `BASELINE_DEBT` **เท่ากับ** ค่าต่ำสุดเสมอ (ห้าม "ไม่เกิน") + บังคับว่าความจุห้ามเหลือเกิน 1 ช่อง (จุดตึงถี่เกิน = กดดัน ไม่ใช่จังหวะ)
   🔴 **ป้าย `tension` = คำกล่าวอ้าง ต้องมีของจริงรองรับ** — เทสต์อ่าน `MarketSizerPanel.tsx` ตัวจริงว่ามีคำถามปิดท้าย และไม่มีคำสัญญาผลลัพธ์เหลืออยู่
   ⚠️ **แต่จุดตึงนี้อยู่ที่ 12,045px ≈ จอที่ 19** (วัดจริงบน 390×664) — 86% ไม่เลื่อนเลย ⇒ ปรับปรุงจริง แต่คนที่ได้รับผลยังน้อย
🔀 **หน้าเดียวกันแสดงได้ 8 แบบ** (A/B holdout `abShowNew` × A/B ลำดับบล็อก `layoutAb` × `persona`) — **ตรวจแบบเดียว = ตรวจไม่ครบ**
   และก้อน `explain`/`proof` ที่สลับกันต้อง **ยาวเท่ากัน** ไม่งั้นก้อนที่อยู่ท้ายจะพาหางไปต่อกับช่วงถัดไป ⇒ หนี้ต่างกันตามกลุ่ม A/B = **เทียบผล A/B ไม่ได้**
กลไก: `emotionalArc.test.ts` (21 เทสต์) อ่าน `LandingPage.tsx` ตัวจริง · เดินครบทั้ง 8 แบบ · เทียบป้ายกับ `data-sec` ทุกตัว (regex ต้องยอมตัวเลขด้วย — เคยมองข้าม `how_30s` ทั้งบล็อก)
...
GOTCHA #6 — แท็ก utm ตายกลางทาง ระหว่างลิงก์สั้น → บทความ → /start (ledger #24)
  ...
  • 🔴 **แก้ 10 ก.ย. 2569 — `seg` ก็ตายกลางทางแบบเดียวกัน แต่กลไกเดิมมองไม่เห็น** (ledger #73)
    วัดจริง: `landing_funnel.seg = default` **154/158 (97.5%)** ⇒ พาดหัวเฉพาะกลุ่ม 9 แบบใน
    `heroVariant` แทบไม่เคยถูกส่งถึงใคร ทั้งที่แคปชั่นเราเขียน `?seg=` มาให้แล้วทุกชิ้น
    ตายสาม hop: ① `shortLinkTarget` สร้าง URL ใหม่จาก path เปล่า ② `utmForwardScript` ส่งต่อแค่ 4 คีย์ utm
    ③ 🔴 **ปุ่มบน `/start` ทั้ง 5 ปุ่มเขียนลิงก์ `/` เปล่า ๆ** — และ `/` คือ **หน้าเดียวที่บันทึก
       `landing_funnel`** (`/start` ยิงแต่ GA) ⇒ กลุ่มที่รู้แล้วถูกทิ้งที่ปุ่มสุดท้าย (ledger #74)
    ⇒ แก้เป็น `FORWARD_KEYS = [...UTM_KEYS,'seg']` + `ShortLink.seg` ประจำลิงก์ที่รู้จริงว่าใครกด
       + `heroVariant.homeHrefFor(seg)` เป็นแหล่งเดียวของลิงก์กลับหน้าแรก
    🔴 **`/start` ≠ `/`** — คนละคอมโพเนนต์ (`Root.tsx`: `/start`→`StartLanding` · `/`→`LandingPage`)
       และ **มีแค่ `/` ที่วัดผล** ⇒ ก่อนอธิบายตัวเลขในตาราง ต้องรู้ก่อนว่า **ใครเป็นคนเขียนแถวนั้น**
    ⚠️ **ลิงก์กลาง ๆ ห้ามเดา seg** (`/ai`·`/ตรวจ`·`/ซิม`) — ยัดพาดหัวผิดกลุ่ม แย่กว่าพาดหัวกลาง ๆ
    ⚠️ **97.5% นี้วัดบั๊กของเรา ไม่ได้วัดตลาด** — ห้ามอ่านว่า "คนที่เข้ามาไม่มีกลุ่ม"
    ⚠️ 🟢 บั๊กทั้งสามข้อ **ตรวจแล้วจริง** · 🟡 **แต่ยังพิสูจน์ไม่ได้ว่าอธิบาย 97.5% ได้เท่าไร**
       (ต้องรอตัวเลขหลัง deploy — วัดที่ `landing_funnel.seg` ที่ไม่ใช่ `default`)
    กลไก: `attributionContract.test.ts` เดิน **เส้นทางเต็มจริง** (คลิป → ลิงก์สั้น → บทความ →
    บทความที่สอง → `/start`) แล้วยืนยันด้วย **ข้อความพาดหัวที่ผู้ใช้เห็น** ไม่ใช่ชื่อคีย์
```
...
src/lib/financialTruth.ts      — ชั้นความจริงของตัวเลขการเงิน + กรวยเศรษฐศาสตร์ (เจ้าของสั่ง 6 ก.ย. 2569)
                                 🔴 **แกนที่สาม ไม่ใช่ taxonomy ซ้ำ** — `ThresholdStatus` ถามว่า *เกณฑ์พิสูจน์แล้วยัง* ·
                                    `ClaimStatus` ถามว่า *คำกล่าวอ้างเชื่อได้แค่ไหน* · `TruthLevel` ถามว่า **ตัวเลขมาจากไหน**
                                    (measured/derived/assumed/placeholder/unavailable)
                                 🔴 กลไกหลัก: **ค่าที่คำนวณ แข็งแรงกว่าวัตถุดิบที่อ่อนที่สุดไม่ได้**
                                    LTV จาก renewal 50% ที่เดาเอง = ตกชั้นเป็น `assumed` อัตโนมัติ แม้สูตรถูกทุกบรรทัด
                                    ⇒ `showAsActual()` = false ⇒ **ห้ามแดชบอร์ดแสดงเป็น Actual** (คำสั่งเจ้าของ ทำเป็นกลไกแล้ว)
                                 🪤 `cacTrap()` — CPA 700 ดูดี แต่ CAC จริง 14,000 (งบ 70,000 · จ่าย 100 · ซื้อหลัก 5)
                                    🔴 ซื้อสินค้าหลัก 0 คน = `trap` ไม่ใช่ `ok` — CPA สวยที่สุดตอนที่แย่ที่สุด
                                 🚪 `INVESTMENT_GATES` 0–5 · `canRelease` = **false เสมอ** ทุกสถานะ (คนอนุมัติเท่านั้น)
                                    ⚠️ คนละชุดกับ `releaseGates` (schema) และ `founderMindset` (เจตนา) — อย่าสับสน
                                 ⭐ North Star = `verifiedFirstOutcomeRate` — **ผลลัพธ์ที่ตรวจสอบได้** ไม่ใช่ "เข้าใช้งานแล้ว"
                                 ⚠️ 2% = เส้นรอดตามโมเดล **ห้ามเป็นเป้าหมาย** · 15% Base Case = **สมมติ** จนมี cohort จริง
                                 🔴 **ราคาในโมเดลยังไม่ตรงกับระบบ** (ยังไม่ฝังลงโค้ด · รอเจ้าของตัดสิน):
                                    · `฿1,990` ในระบบ = **ISO Readiness Pilot** (ประตูข้าง) ไม่ใช่ประตูหน้าของกรวยหลัก
                                      ⇒ ถ้าจะเปลี่ยนบทบาท ต้องแก้กฎ `sideDoor` ที่ freeze ไว้ว่าห้ามเป็นเครื่องยนต์
                                    · `฿12,000/ปี` **ไม่มีอยู่ในระบบ** — แพ็กจริง 790/1,490/5,900 ต่อ**เดือน**
                                 🟢 **รายปีแก้แล้ว 11 ก.ย. 2569** (เจ้าของสั่ง "ราคาใช้ล่าสุด" → ไล่ดูของจริงแล้วเจอว่ารายปีหลุดเกณฑ์)
                                    เดิม `ANNUAL_MONTHS_CHARGED = 10` (ลด 16.7%) ⇒ starter 7,900 = **31.1%** · growth 14,900 = **27.0%**
                                    · scale 59,000 = **26.2%** — ต่ำกว่า `MIN_MARGIN_PCT` 32% **ทั้ง 3 แพ็ก** · `Billing.tsx` ขายจริงอยู่
                                    ⇒ **แก้เป็น 11 เดือน (ลด 8.3%)** ⇒ 37.3 / 33.6 / 32.9% ผ่านครบ · **ราคารายเดือนไม่ถูกแตะสักบาท**
                                    🔒 เทสต์บังคับ **สองทาง**: ค่าที่ใช้ต้องผ่านเกณฑ์ **และ** ค่าที่น้อยกว่า 1 ต้องไม่ผ่าน
                                       ⇒ กันทั้ง "ลดจนขาดทุน" และ "ขี้เหนียวเกินจำเป็น" · ห้ามถอยกลับไป 10 (เทสต์ล็อก)
                                    🔴 **ราคาเคยเขียนไว้ 3 ที่** — `access.PLAN_PRICE_NUM` · `tokenEconomics.PLAN_PRICE_THB` ·
                                       **`SalePage.tsx` ที่ "mirror" ไว้เป็นเลขตายตัว 5 ตัว** (คำว่า mirror ในคอมเมนต์ ไม่ได้ทำให้มันเป็น mirror)
                                       ⇒ ตอนนี้ SalePage คำนวณสดจาก `access.ts` · สองตัวแรกมีเทสต์บังคับให้ตรงกัน (ledger #76)
                                 🔴 ต้องบันทึกเวลา 6 ตัว (paid_at · first_login_at · first_job_completed_at …)
                                    **ยังไม่มีสักตัว** ⇒ แตะ schema = ด่านปล่อยของกั้น ⇒ Gate 1–2 ผ่านไม่ได้
                                    เพราะ **เรามองไม่เห็น** ไม่ใช่เพราะลูกค้าไม่ activate (`unknown` ≠ `open`)
                                 เอกสาร: [FINANCIAL-TRUTH-2026-09-06.md](docs/product/FINANCIAL-TRUTH-2026-09-06.md)
