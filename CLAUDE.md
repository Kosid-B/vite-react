...
src/lib/financialTruth.ts      — ชั้นความจริงของตัวเลขการเงิน + กรวยเศรษฐศาสตร์ (เจ้าของสั่ง 6 ก.ย. 2569)
                                 ...
                                 🔴 **แต่ "รายปี" มีอยู่แล้ว และหลุดเกณฑ์กำไร** (เจอ 10 ก.ย. 2569 ตอนเจ้าของสั่ง "ราคาใช้ล่าสุด")
                                    `access.ANNUAL_MONTHS_CHARGED = 10` (จ่าย 10 ใช้ 12 ≈ ลด 16.7%) · `Billing.tsx` ขายจริง
                                    วัดที่เพดาน token เต็ม: starter 7,900 = **31.1%** · growth 14,900 = **27.0%** · scale 59,000 = **26.2%**
                                    ⇒ ต่ำกว่า `MIN_MARGIN_PCT` 32% **ทั้ง 3 แพ็ก** · ค่าที่ผ่านครบ = **11 เดือน** (ลด 8.3%)
                                    🔴 **ยังไม่แก้ — เปลี่ยนราคาที่ขายอยู่ = คำตัดสินของเจ้าของ** ประกาศเป็นหนี้ไว้ใน
                                       `annualPricing.test.ts` (`KNOWN_BELOW_GATE`) ⇒ แย่ลงไม่ได้ · ลืมไม่ได้
                                    ⚠️ ราคาเขียนไว้ 2 ที่ (`access.PLAN_PRICE_NUM` + `tokenEconomics.PLAN_PRICE_THB`) — เทสต์บังคับให้ตรงกันแล้ว
