...
2. **เพิ่ม billing รายปี (1 เดือนฟรี = ลด 8.3%)** — anchor ราคา + เงินสดล่วงหน้า + ลูกค้า commit เร็วขึ้น (Starter **฿8,690**/ปี · Growth **฿16,390**/ปี · Scale **฿64,900**/ปี) → *value-prop lever อันดับ 1*
   > 🔴 **แก้ 11 ก.ย. 2569**: เดิมข้อนี้เขียน *"2 เดือนฟรี = ลด 16.7%"* ซึ่งทำให้ margin รายปี **หลุดเกณฑ์ `MIN_MARGIN_PCT` 32% ทั้ง 3 แพ็ก** (31.1 / 27.0 / 26.2%) · เจ้าของอนุมัติแก้เป็น 11 เดือน
   > ⚠️ และตัวเลขเดิมในวงเล็บยัง **พิมพ์ผิด** ด้วย — เขียน Starter ฿5,900/ปี ทั้งที่ 790 × 10 = **฿7,900**
   > (฿5,900 คือราคา *รายเดือน* ของ Scale) ⇒ เอกสารนี้เคยแนะนำราคาที่ไม่มีอยู่จริง · ตอนนี้ทุกตัวเลขคิดจาก `access.annualPrice()` แล้ว (`annualPricing.test.ts` เฝ้า)
