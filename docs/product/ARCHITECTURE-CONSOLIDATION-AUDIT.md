...
🟢 **แก้แล้ว 24 ส.ค. 2569** — ทุกจุดเรียกผ่านประตูเดียว `lib/aiAssist.callAiAssist()`
ซึ่งเติม `governingBlock()` (กติกาย่อ ~1.2k ตัวอักษร) ให้อัตโนมัติ
· จุดที่ไม่รับกติกาต้องระบุเหตุผลจากรายการที่กำหนดไว้ (`UNGOVERNED_REASON`) — เขียนมั่วไม่ได้
· ⚠️ ไม่ส่ง `brandBriefBlock()` เต็มก้อน (11,018 ตัวอักษร) เพราะแปะทุกคำขอ = เผาโควตา
· `constitutionReach.test.ts` บังคับว่า **ห้ามมีใครเรียก `invoke('ai-assist')` เองนอกประตู** (ยืนยันแดงจริงแล้ว)
· 🟡 ยังไม่ครอบคลุม `ai-plan` / `agent-run` — ต้องแก้ฝั่ง edge function ซึ่งต้อง deploy มือ
...
3. ⚠️ **กฎห้ามแตะ schema (ด่านปล่อยของ · `src/lib/releaseGates.ts`) ถูกใช้กับรีโปนี้เท่านั้น** — schema ของ production ถูกเปลี่ยนไปแล้ว 10 ครั้งเมื่อ 23 ส.ค. โดยอีกระบบหนึ่ง
...
⚠️ **ทางแก้ที่ยังทำไม่ได้**: ทำเป็นตาราง DB — ด่านปล่อยของยังกั้นอยู่ (`src/lib/releaseGates.ts` · สถานะสดอยู่ที่นั่น)
...
| **0** | D · ทำ Genome เป็นตาราง DB | ⛔ **พักไว้: ด่านปล่อยของยังกั้น** (`src/lib/releaseGates.ts`) |
