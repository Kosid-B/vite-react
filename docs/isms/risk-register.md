...
| **R3** | บัญชีแอดมิน (`support@b-tctraining.com`) **ไม่มี MFA** → ถูกยึดบัญชีเข้าถึงทุก workspace (⚠️ ตรวจ `auth.users` 2026-07-05 พบว่า production ยังไม่มีผู้ใช้ลงทะเบียนเลยสักคนตอนตรวจ — บัญชีแอดมินสร้างแล้วผ่าน invite 2026-07-05 `user_id=9a4fb9bb-...`) | C/I/A | 2 | 3 | 🔴 6 สูง | Modify | 8.5, 5.15 | ผู้บริหาร | 🟡 บัญชีสร้างแล้ว (invite email ส่งไปที่ `support@b-tctraining.com` — รอผู้ถือบัญชีตั้งรหัสผ่านผ่านลิงก์ในอีเมล) — **ยังต้องเปิด MFA เองหลัง login ครั้งแรก** (Supabase Auth ต้องให้เจ้าของบัญชีทำเอง ไม่มี API ให้ผู้ดูแลตั้งแทนได้) |
