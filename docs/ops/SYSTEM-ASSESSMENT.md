...
1. **apply `0028` handoff_nonces บน prod** — ตารางยังไม่มีบน prod · latent (gate `theossphereLive` ยังไม่ live) แต่ต้อง apply **ก่อน** เปิด theossphere handoff
2. **reconcile migration ledger prod↔repo** — prod บันทึก migration ตามชื่อตรง แต่เลข version เพี้ยน (offset ~3 ตั้งแต่ 0018) → `supabase db push` อาจชน · (option) บันทึก `0029` ลง ledger prod ด้วย
