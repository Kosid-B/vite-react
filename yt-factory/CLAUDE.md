...
สายการผลิตคลิป YouTube ด้วย AI สำหรับตลาดไทย ผู้ใช้เป็นเจ้าของช่องที่ไม่ออกกล้อง

**สถานะตอนนี้: ใช้เองในบริษัทก่อน ยังไม่ขายให้ลูกค้านอก** — โครง multi-tenant (org + RLS +
เครดิต + คลังโควตา) ทำไว้แล้วและไม่ต้องรื้อ แต่การตัดสินใจให้ยึด "ใช้เองก่อน" เป็นหลัก:
- **ไม่ต้องทำระบบเก็บเงิน** เปิดเครดิตให้ตัวเองผ่าน `grant_credits` พอ
  เครดิตทำหน้าที่ "กันค่า API บานปลาย" ไม่ใช่ "เก็บเงินลูกค้า"
- **ขั้นตอนที่ต้องมีคนกดเองยอมรับได้** ถ้าแลกกับคุณภาพคลิปที่ดีกว่า
  (ข้อนี้จะกลับด้านทันทีถ้าวันหนึ่งเปิดขายจริง — ดูหัวข้อ CapCut)
...
## การซิงก์โค้ดกับเครื่องผู้ใช้ (เตือนทุกครั้ง ห้ามลืม)

ผู้ใช้ทำงานบนเครื่อง Windows ที่ `D:\Youtube Ai Automate` ส่วน Claude ทำงานในคอนเทนเนอร์
คนละเครื่องกัน **git คือสะพานเดียวที่เชื่อมสองฝั่ง** ไม่มีอย่างอื่น — ไฟล์ที่ยังไม่ push
อีกฝั่งมองไม่เห็นเลย

สาขาที่ใช้: `claude/yt-factory-context-d4y1cq`

**สิ่งที่ต้องทำเสมอ:**

1. **ทุกครั้งที่ push เสร็จ ต้องบอกผู้ใช้ให้ดึงลงเครื่อง** พร้อมคำสั่งเต็ม อย่าบอกแค่ว่า
   "push แล้ว" เพราะบนเครื่องเขายังเป็นของเก่าอยู่
   ```powershell
   cd "D:\Youtube Ai Automate\yt-factory"
   git pull origin claude/yt-factory-context-d4y1cq
   ```
2. **ก่อนเริ่มแก้ไฟล์ที่ผู้ใช้อาจแก้เองบนเครื่อง ให้ถามก่อนว่า commit + push มาหรือยัง**
   ถ้าไม่ถามแล้วผมแก้ทับ งานเขาจะชนกันตอน pull
3. **ผู้ใช้ส่ง error ที่ไม่ตรงกับโค้ดที่เห็น = สงสัยเรื่องเวอร์ชันไม่ตรงกันก่อนเป็นอันดับแรก**
   ให้เขารัน `git log --oneline -1` เทียบ commit hash กับฝั่งเรา ก่อนจะไปไล่หาสาเหตุอื่น
   (เสียเวลาไล่บั๊กที่แก้ไปแล้วเป็นความผิดพลาดที่เจ็บที่สุด)
4. **ผู้ใช้แก้เองบนเครื่องต้อง commit + push กลับมา** ไม่งั้น Claude มองไม่เห็นและจะแก้
   ทับของเขา — เตือนข้อนี้ทุกครั้งที่เขาบอกว่าแก้ไฟล์เอง
...
ภาพ    : Pexels (ฟรี 200 คำขอ/ชม. 20,000/เดือน) — AI image gen ทำให้เกินงบทันที
```

### เครดิตช่างภาพ — ห้ามข้าม

เราแจ้ง Pexels ตอนขอ API key ว่าจะให้เครดิตช่างภาพในคำอธิบายใต้คลิปทุกคลิป
ทุกภาพที่ดาวน์โหลดจึงต้องบันทึกชื่อช่างภาพ + ลิงก์หน้าภาพลง `video_assets` เสมอ
แล้ว `video_render` ต่อบล็อกเครดิตเข้า `videos.description` ให้อัตโนมัติ
(Pexels ไม่ได้บังคับเครดิต แต่บังคับถ้าจะขอโควตาไม่จำกัด และเราแจ้งเขาไปแล้วว่าจะทำ)

**Pexels ค้นภาษาอังกฤษ ฉากเราเป็นภาษาไทย** — ยิงข้อความไทยเข้าไปตรง ๆ ได้ภาพมั่ว
`sceneImageQueries()` เรียกโมเดลครั้งเดียวแปลงทุกฉากเป็นคำค้นอังกฤษพร้อมกัน
(เรียกรวมทีเดียวเพื่อให้โมเดลเลี่ยงคำค้นซ้ำระหว่างฉากได้ และถูกกว่าเรียกฉากละครั้ง)
...
ยืนยันแล้วว่า ffmpeg 6.1 + ฟอนต์ Loma เรนเดอร์ซับไทยได้ถูกต้อง สระและวรรณยุกต์ไม่หลุด

### กับดัก ffmpeg ที่เจอมาแล้ว (อย่าให้เสียเวลาซ้ำ)

> 📌 ข้างล่างคือสรุปย่อ · **ฉบับเต็มพร้อมคำสั่งตรวจสอบอยู่ที่ skill `thai-video-ffmpeg`**
> (`.claude/skills/thai-video-ffmpeg/SKILL.md`) — เรียกใช้ทุกครั้งที่จะแก้ `src/lib/ffmpeg.ts`,
> `render-plan.ts`, `worker/handlers/video-render.ts` หรือเจอปัญหาซับ/ภาพ/เวลาในคลิป
> เจอกับดักใหม่ให้เขียนกลับไปที่ skill นั้น ไม่ใช่ที่นี่ (กันข้อมูลสองที่ไม่ตรงกัน)

- **`zoompan` นับเป็นเฟรม ไม่ใช่วินาที** ต้องแปลงความยาวฉาก × fps ก่อนเสมอ
- **ต้องขยายภาพ ×4 ก่อนเข้า `zoompan`** ไม่งั้นภาพกระตุกเป็นขั้น ๆ
  เพราะ zoompan ปัดตำแหน่งเป็นพิกเซลเต็มของภาพต้นทาง
- **`offset` ของ `xfade` นับสะสมจากต้นสายที่ต่อมาแล้ว** ไม่ใช่จากต้นฉากถัดไป
  คิดผิดข้อนี้แล้วฉากหลัง ๆ จะเลื่อนสะสมไปเรื่อย ๆ
- **ภาพนิ่งต้องมี `-loop 1` และ `-t`** ไม่งั้น ffmpeg อ่านเฟรมเดียวแล้วจบ
- **ใส่ `subtitles` หลัง `xfade` เสมอ** ถ้าใส่ก่อน ตัวหนังสือจะถูกซูมไปกับภาพ
- **ตัดความยาวด้วย `-t` ที่ผลรวมเสียง** เพราะภาพฉากท้ายมีหางเฟดเผื่อไว้
- **โหมดพังที่เงียบที่สุด**: `subtitles` ทำงานผ่านแต่ไม่วาดอะไรเลย (พาธผิด/ฟอนต์ไม่มีอักษรไทย)
  ไฟล์ออกมาปกติทุกอย่างยกเว้นไม่มีตัวหนังสือ — มีเทสต์วัดความสว่างแถบล่างจับไว้แล้ว
- **ค่าออปชันของ filter ต้อง escape สองชั้น ไม่ใช่ชั้นเดียว** — ffmpeg แกะสตริงสองรอบ
  (ระดับ filtergraph แล้วค่อยระดับออปชันของ filter) `\` ชั้นเดียวถูกกินหมดตั้งแต่รอบแรก
  พังกับพาธ Windows ทุกครั้งเพราะขึ้นต้นด้วย `C:\` — ffmpeg อ่านชื่อไฟล์เป็นแค่ `"C"`
  แล้วโยนที่เหลือไปเป็นออปชันถัดไป ล้มด้วย `Unable to parse ... as image size`
  ⚠️ บั๊กนี้เงียบสนิทบน Linux เพราะพาธ POSIX ไม่มี `:` และ `\` — มีเทสต์สร้างโฟลเดอร์
  ชื่อ `C:\Users\ทดสอบ` จำลองไว้จับฝั่ง POSIX แล้ว (Windows สร้างชื่อแบบนี้ไม่ได้
  และไม่ต้องสร้าง เพราะได้ `C:\` ติดมากับ temp path อยู่แล้ว)

### ทำไมไม่ใช้ CapCut แทน ffmpeg (ตรวจแล้ว ส.ค. 2569)

CapCut **ไม่มี public API สำหรับ render / ทำซับ / batch export** — "Open Platform" ของเขาคือ
ปลั๊กอินที่รันข้างในตัวแอป ส่วน AI API จำกัดแค่ text-to-video กับค้นเทมเพลต
worker เรียกไม่ได้ · ต่อให้คุมแอปเดสก์ท็อปด้วยหุ่นยนต์กดจอ ก็ต้องมีเครื่องมีจอเปิดค้าง
พังทุกครั้งที่ CapCut อัปเดต และรับงานหลายชิ้นพร้อมกันไม่ได้

ทางที่ใช้ CapCut ได้จริงคือ **ส่งออกเป็นไฟล์ draft ให้เปิดแก้ต่อเอง**
(มีเครื่องมือโอเพนซอร์สของชุมชนทำได้ แต่รูปแบบ draft ไม่มีเอกสารทางการ พังได้เมื่อแอปอัปเดต)

**ตัดสินใจแล้ว: ใช้ ffmpeg** (เจ้าของโปรเจคเลือก ส.ค. 2569)
เหตุผล: รูปแบบ draft ของ CapCut ไม่มีเอกสารทางการและพังเมื่อแอปอัปเดต ส่วน ffmpeg
อัตโนมัติเต็มและไม่ต้องรื้อถ้าวันหนึ่งเปิดขาย · CapCut draft ยังทำเป็นทางเลือกเสริมได้ทีหลัง

`src/lib/render-plan.ts` แยกตรรกะไทม์ไลน์ออกจากโปรแกรมปลายทาง
เพิ่มปลายทางใหม่ = เขียนตัวแปลงจาก RenderPlan ตัวเดียว ไม่ต้องแตะตรรกะการวางเวลา
...
2. `video_render` handler — ต่อครบแล้ว เหลือทดสอบกับ API จริง:
   - [x] แบ่งสคริปต์เป็นฉาก (`src/lib/scenes.ts`)
   - [x] ซับไตเติลจากความยาวเสียงจริง (`src/lib/subtitles.ts`)
   - [x] เรียก TTS จริง + อ่านความยาวไฟล์เสียง (`src/lib/tts.ts`)
   - [x] หาภาพ stock ต่อฉาก (`src/lib/pexels.ts`)
   - [x] ประกอบด้วย ffmpeg (Ken Burns + เสียง + ซับ) — `src/lib/ffmpeg.ts`
   - [ ] ปกคลิป (ยังไม่ทำ)
3. `metrics_sync` ดึง CTR / AVD / RPM จาก YouTube Analytics API ลง `video_metrics` (รอข้อ 1)
4. Realtime subscription บน `jobs` ให้บอร์ด dashboard ขยับเอง

**เลื่อนออกไปเพราะยังใช้เองในบริษัท** (หยิบกลับมาเมื่อจะเปิดขาย):
- ระบบเก็บเงิน / Stripe webhook — ตอนนี้เปิดเครดิตเองผ่าน `grant_credits` พอ
- ตัวแปลง RenderPlan → CapCut draft (ทางเลือกเสริมให้คนอยากแต่งคลิปเอง)

## เสียงบรรยาย (TTS)

```
ผู้ให้บริการ : Google Cloud Text-to-Speech · ตั้ง GOOGLE_TTS_API_KEY
รูปแบบเสียง  : LINEAR16 (WAV) ไม่ใช่ MP3
เพดานคำขอ   : 5,000 "ไบต์" ต่อคำขอ — ไทยกินตัวละ 3 ไบต์ = ~1,600 ตัวอักษร
```

**ทำไมต้อง WAV ไม่ใช่ MP3**: อ่านความยาวเสียงได้จากหัวไฟล์เลย ไม่ต้องพึ่ง ffprobe
ซับไตเติลต้องใช้ความยาวจริงของทุกฉาก ถ้าอ่านไม่ได้ก็ทำซับไม่ได้ · ไฟล์ใหญ่กว่าแต่เป็นไฟล์ชั่วคราว

`splitIntoScenes()` ตัดฉากมาที่ ~210 ตัวอักษร จึงไม่มีทางชนเพดาน 5,000 ไบต์ (มีเทสต์คุมไว้)
อย่าเดาชื่อเสียงไทยเอง — เรียก `listThaiVoices()` ดูรายชื่อจริงตอนตั้งค่า
