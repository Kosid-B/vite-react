...
const TOOL_ITEMS: { id: PageId; label: string; icon: string; desc: string; stage: string }[] = [
  ...
  { id: 'trustcontent', label: 'คอนเทนต์สายเชื่อใจ (T.R.U.S.T.)', stage: 'วางแผนการตลาด', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z',
    desc: 'ขั้น 6.5 · ร่างคอนเทนต์อย่างมีจริยธรรมด้วย T.R.U.S.T. — Target·Resonate·Unique·Social proof·Track' },
  ...
  return (
    <nav className={`sidebar${isOpen ? ' open' : ''}${collapsed ? ' collapsed' : ''}`}>
      ...
      <div className="nav-section">
        ...
        <button className={`nav-item ${activePage === 'knowledge' ? 'active' : ''}${locked('knowledge') ? ' nav-locked' : ''}`} onClick={() => onNavigate('knowledge')}>
          <svg className="nav-ico" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8">
            <path d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.247m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.247" />
          </svg>
          คลังความรู้ ISO/PDPA
          {locked('knowledge') ? <span className="nav-lock">🔒</span> : <span className="nav-dot" />}
        </button>
      </div>
