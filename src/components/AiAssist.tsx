...
const PAGE_INFO: Record<PageId, { label: string; prompts: string[]; context: (d: AppData) => string }> = {
  ...
  pulse: { label: 'Pulse & A/B', prompts: ['อ่านผล pulse ของฉันแล้วแนะนำว่าควรทำอะไรต่อ', 'อธิบายว่าการทดลอง A/B นี้วัดอะไร', 'ทำไมบางวันฉันรู้สึกไม่อยากใช้งานต่อ'], context: d => { const e = d.experiments; const ps = e?.pulses ?? []; const avg = ps.length ? (ps.reduce((s, p) => s + p.score, 0) / ps.length).toFixed(2) : '—'; return `Pulse: ยินยอมวัดผล=${e?.enabled ? 'ใช่' : 'ไม่'}, จำนวน pulse=${ps.length}, คะแนนเฉลี่ย=${avg}/3`; } },
