  ...
  return (
    <section style={{ padding: '20px 24px' }}>
      ...
      <div style={{ maxWidth: 920, margin: '0 auto', borderRadius: 16, border: `1px solid ${C.border}`, background: C.panel, padding: '22px 24px' }}>
        ...
        {/* ── จุดตึง: เปิดช่องว่างด้วย "ตัวเลขของเขาเอง" ไม่ใช่คำสัญญาของเรา ──
             🔴 ของเดิมปิดท้ายด้วยคำสัญญาว่าโอกาสก้อนนี้เป็นของเขาแน่ ๆ ซึ่งผิด 2 ชั้น:
                (ไม่ยกประโยคเดิมมาเขียนตรง ๆ — เทสต์สแกนไฟล์นี้ ถ้ายกมาก็จะจับตัวเองตลอดกาล
                 แล้วเราจะแก้ด้วยการยกเว้นตัวเอง = ช่องโหว่ถาวร · บทเรียนเดียวกับ releaseGates)
                ① เป็นคำสัญญาถึงผลลัพธ์ที่เราไม่มีหลักฐาน (ยังไม่มีลูกค้าจ่ายจริงสักราย)
                ② มันปิดเรื่องให้ผู้อ่าน "อิ่ม" ⇒ ไม่มีเหตุผลจะอ่านต่อ
                   (emotionalArc: ความโล่งที่ไม่ได้คลายอะไร = ทางตัน · ตอนจบต้อง "ค้าง" ไม่ใช่ "อิ่ม")
             ตอนนี้ปิดด้วยคำถามที่ **เขาตอบเองไม่ได้ถ้าไม่รู้ตัวเลขของตัวเอง**
             — เงื่อนไขเดียวกับ `commentReply.videoEnding` และไม่มีอะไรที่ไม่จริงสักคำ ── */}
        {revealed && (
          <div style={{ borderRadius: 12, border: `1px solid ${C.amber}`, background: 'rgba(245,158,11,0.08)', padding: '12px 14px', marginBottom: 10 }}>
            <div style={{ fontSize: 14.5, fontWeight: 800, color: C.white, lineHeight: 1.5 }}>❓ แล้วปีที่ผ่านมา คุณได้ไปเท่าไหร่จากก้อนนี้</div>
            <div style={{ fontSize: 12.5, color: C.slate, marginTop: 5, lineHeight: 1.65 }}>
              ตอบเป็นตัวเลขได้ไหม — ถ้ายังตอบไม่ได้ ก็ยังบอกไม่ได้ว่าปีนี้โตขึ้นหรือเท่าเดิม
              และแผนหาลูกค้าที่วางบนตัวเลขที่ไม่รู้ ก็คือการเดา
            </div>
          </div>
        )}
