...
export default function PriorityActions({ data, onUpdate }: Props) {
  ...
  useEffect(() => { document.querySelectorAll<HTMLTextAreaElement>('.a-desc').forEach(autoH); });
  ...
  return (
    <div>
      ...
      <div className={`celebration ${allDone ? 'show' : ''}`}>
        ...
        <div className="celebration-sub">ทีมของคุณพร้อมแล้วสำหรับขั้นตอนถัดไป<br />ลองเพิ่ม action ใหม่หรือ reset เพื่อเริ่มรอบถัดไป</div>
      </div>
      <div className="action-list">
        {data.actions.map((a, ai) => (
          <div key={ai} className={`a-item priority-${a.priority} ${a.done ? 'done' : ''}`}>
            <div className={`a-check ${a.done ? 'checked' : ''}`} onClick={() => toggle(ai)}>
              {a.done && <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>}
            </div>
            <div className="a-num" style={{ background: a.nb, color: a.nt }}>{ai + 1}</div>
            <div className="a-body">
              <input className="a-title" defaultValue={a.title} key={`title-${ai}`} onBlur={e => saveField(ai, 'title', e.target.value)} spellCheck={false} />
              <textarea className="a-desc" rows={2} defaultValue={a.desc} key={`desc-${ai}`} onBlur={e => saveField(ai, 'desc', e.target.value)} onChange={e => autoH(e.target)} ref={el => { if (el) autoH(el); }} spellCheck={false} />
              <div className="a-tags">{a.tags.map((t, ti) => <span key={ti} className={`tag ${t.c}`}>{t.l}</span>)}</div>
            </div>
          </div>
        ))}
      </div>
