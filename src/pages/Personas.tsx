        ...
        {data.personas.map((p, pi) => (
          <div key={pi} className="p-card">
            <div className="p-card-top">
              ...
              <input className="p-name-inp" defaultValue={p.name} key={`name-${pi}`} onBlur={e => saveField(pi, 'name', e.target.value)} spellCheck={false} />
              <input className="p-role-inp" defaultValue={p.role} key={`role-${pi}`} onBlur={e => saveField(pi, 'role', e.target.value)} spellCheck={false} />
            </div>
            <div className="p-quote-wrap">
              <div className="p-quote-mark">&quot;</div>
              <textarea className="p-quote-inp" rows={3} defaultValue={p.quote} key={`quote-${pi}`} onBlur={e => saveField(pi, 'quote', e.target.value)} spellCheck={false} />
            </div>
            {P_SECTIONS.map(sec => (
              <div key={sec.key} className="p-section">
                ...
                {p[sec.key].map((item, idx) => (
                  <div key={idx} className="p-item">
                    ...
                    <input className="p-item-inp" defaultValue={item} key={`${sec.key}-${pi}-${idx}`} onBlur={e => saveItem(pi, sec.key, idx, e.target.value)} spellCheck={false} />
                    <button className="p-item-del" onClick={() => delItem(pi, sec.key, idx)}>×</button>
                  </div>
                ))}
                <button className="add-row" style={{ fontSize: 11, padding: '3px 4px', marginTop: 2 }} onClick={() => addItem(pi, sec.key)}>＋ เพิ่ม</button>
              </div>
            ))}
          </div>
        ))}
