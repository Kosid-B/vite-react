 ...
 * โหมด "อัตโนมัติตามเวลาจริง" จับเวลาจากนาฬิกาเครื่อง + ฤดูจากเดือน (ภูมิอากาศไทย)
 * แถบ XP / ระดับเมือง ผูกกับ "ผลงานจริง" ของบริษัท (cityStats + COMPANY_LEVELS) */
...
const LEVEL_META = [
  { rank: 'Starter', ic: '🌱', name: 'หมู่บ้านสตาร์ทอัป', tier: 'Starter' },
  { rank: 'Growing', ic: '🌿', name: 'ชุมชนกำลังโต', tier: 'Growing' },
  { rank: 'Professional', ic: '⭐', name: 'เมืองมืออาชีพ', tier: 'Professional' },
  { rank: 'Advanced', ic: '🏆', name: 'นครธุรกิจ', tier: 'Advanced' },
  { rank: 'Elite', ic: '👑', name: 'มหานคร AI', tier: 'Elite' },
];
...
export default function CityLevelUp({ data }: { data: AppData; onNavigate?: (p: PageId) => void; onUpdate?: (d: AppData) => void }) {
  ...
  useEffect(() => {
    if (!auto) {
      setAutoStatus('ปรับเอง — กด “อัตโนมัติ” เพื่อกลับมาตามเวลาจริง');
      return;
    }
    const tick = () => {
      const now = new Date();
  ...
  // real XP progress toward the next city tier
  const lvl = s.level;
  ...
  return (
    <div className="clv-root" data-time={time} data-season={season}>
      <style>{CLV_CSS}</style>
      <div className="clv-wrap">
        <div className="clv-head-row">
          <div>
            <div className="clv-eyebrow">Company City · Level Up</div>
            <h1 className="clv-h1">ความเจริญ <span className="clv-hl">สู่ระดับถัดไป</span></h1>
            <p className="clv-lede">
              เมืองบริษัทของคุณเติบโตตามผลงานจริง — ทุกงานที่ทีม AI ทำเสร็จ ทุกดีลที่ปิด และทุกบาทของกำไร
              ยกเมืองจาก “หมู่บ้านสตาร์ทอัป” ขึ้นสู่ “มหานคร AI”. นี่คือช่วงกระโดดสู่ระดับถัดไป.
            </p>
          </div>
          ...
          <div className="clv-controls">
            ...
            <div className="clv-ctrl-group">
              <span className="clv-ctrl-label">ช่วงเวลา · แสงและเงา</span>
            ...
            <div className="clv-ctrl-group">
              <span className="clv-ctrl-label">ฤดูกาล</span>
          ...
          {COMPANY_LEVELS.map(l => {
            ...
            return (
              <div key={l.rank} className={`clv-lvl${isActive ? ' clv-active' : ''}${isGoal ? ' clv-goal' : ''}`}>
                ...
                <div className="clv-tier">{meta.tier}</div>
...
/* Scoped styles (prefixed .clv-) — palette vars swap by [data-time] on the root.
   Kept inline so the whole page ships as a single component. */
const CLV_CSS = `
.clv-root{ --bg1:#070d24; --bg2:#0d1638; --panel:#0e1732; --panel-border:rgba(120,140,220,.18);
  ...
  color:var(--text); min-height:100%; border-radius:18px; transition:background .6s ease,color .4s ease; }
...
.clv-wrap{ max-width:1180px; margin:0 auto; padding:36px 24px 48px; }
...
.clv-h1{ font-size:clamp(30px,4.6vw,52px); font-weight:800; line-height:1.15; margin:14px 0; color:var(--text); }
.clv-hl{ color:var(--gold); text-shadow:0 0 34px var(--glow); }
.clv-lede{ max-width:560px; color:var(--text-soft); font-size:16px; line-height:1.75; }
.clv-head-row{ display:flex; justify-content:space-between; align-items:flex-start; gap:24px; flex-wrap:wrap; }

.clv-controls{ display:flex; flex-direction:column; gap:14px; align-items:flex-end; }
.clv-ctrl-group{ display:flex; flex-direction:column; gap:8px; align-items:flex-end; }
.clv-ctrl-label{ font-size:12px; font-weight:600; letter-spacing:.14em; color:var(--text-soft); text-transform:uppercase; }
...
.clv-tbtn{ display:flex; align-items:center; gap:6px; cursor:pointer;
  ...
  border-radius:999px; padding:7px 13px; font-family:inherit; font-size:13px; font-weight:600;
...
.clv-auto-btn{ font-size:13.5px; padding:8px 16px; }
...
.clv-progress-top{ display:flex; justify-content:space-between; align-items:baseline; gap:16px; flex-wrap:wrap; margin-bottom:14px; }
.clv-progress-title{ font-size:15px; color:var(--text-soft); }
.clv-progress-title b{ color:var(--text); }
.clv-xp{ font-size:19px; font-weight:800; font-variant-numeric:tabular-nums; letter-spacing:.02em; color:var(--text); }
.clv-bar{ height:12px; border-radius:999px; background:var(--bar-track); overflow:hidden; }
...
.clv-levels{ display:grid; grid-template-columns:repeat(auto-fit,minmax(180px,1fr)); gap:16px; margin-top:22px; }
.clv-lvl{ position:relative; background:var(--card); border:1px solid var(--card-border);
  border-radius:16px; padding:22px 20px 18px; transition:transform .18s ease,box-shadow .2s ease; }
.clv-lvl:hover{ transform:translateY(-3px); }
.clv-ic{ font-size:26px; margin-bottom:12px; }
.clv-lvl-h3{ font-size:17px; font-weight:700; margin-bottom:3px; color:var(--text); }
.clv-tier{ font-size:11.5px; letter-spacing:.22em; color:var(--text-soft); text-transform:uppercase; font-weight:600; }
.clv-range{ margin-top:10px; font-size:13.5px; color:var(--text-soft); font-variant-numeric:tabular-nums; }
...
.clv-tag{ position:absolute; top:-11px; left:16px; font-size:11.5px; font-weight:700; padding:4px 12px; border-radius:999px; white-space:nowrap; }
.clv-tag.clv-here{ background:var(--active-border); color:#fff; }
.clv-tag.clv-next{ background:var(--goal-border); color:#1c1503; }

@media (max-width:720px){
  .clv-wrap{ padding:28px 14px 40px; }
