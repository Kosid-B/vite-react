...
export const PAID_CLIPS_PER_MONTH: Record<Exclude<VideoPlan, 'free'>, number> = {
  starter: 1, growth: 2, scale: 7,
};

/* 📌 สิ่งที่ตัวเลขชุดนี้บอก (เจ้าของยกเกณฑ์เป็น >32% เมื่อ 22 ส.ค. 2569):
 *   แพ็กเดิมทั้งสามตัว **แบกวิดีโอไม่ไหว** — ราคาถูกกินไปกับโควตา token หมดแล้ว
 *     starter ฿790  → เหลือ 1 คลิป · growth ฿1,490 → 2 คลิป · scale ฿5,900 → 7 คลิป
 *   ⇒ วิดีโอเป็น "ของแถม" ในแพ็กเดิม ไม่ใช่ผลิตภัณฑ์
 *   ⇒ ถ้าอยากให้วิดีโอเป็นผลิตภัณฑ์จริง ต้องมีแพ็กที่ออกแบบมาเพื่อมัน = VIDEO_PLAN ด้านล่าง
 *      (นี่คือเหตุผลเชิงตัวเลขที่สนับสนุนไอเดีย ฿1,790 ของเจ้าของ ไม่ใช่แค่ความเห็น) */
 ...
 *
 *    ฿1,790 × 66% (กันกำไร 34% ตามเป้า)   = ฿1,181 คืองบต้นทุนทั้งหมดที่ใช้ได้
 *  − token 1.5M (เท่าแพ็ก starter)        = ฿454
 *  = เหลือให้วิดีโอ ฿727 ÷ ฿35            = 20 คลิป
 *  ⇒ ตั้งที่ **20 คลิป** = 1 คลิปทุกวันทำการ · มาร์จิน 35.6% (เหนือเกณฑ์ >32% อยู่ 3.6%)
 *     22 คลิปจะเหลือ 31.6% = ตกเกณฑ์ ⇒ 20 คือตัวเลขที่ปลอดภัยและยังเล่าเรื่องได้
 *
