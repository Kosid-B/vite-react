...
describe('theme-safe-color — เครื่องมือตรวจต้องเดินครบ', () => {
  ...
  /* 🔴 กติกาข้อนี้ถูก "ยกระดับ" 27 ส.ค. 2569 — ไม่ใช่ถูกยกเลิก
   *   เดิมบังคับว่าตัวตรวจต้อง **ข้าม** ชั้นโปร่งแสง (`g.a > 0.5`)
   *   ซึ่งกันความผิดแบบแรกได้ (อ่าน rgba(...,.08) เป็นสีทึบ ⇒ รายงาน "เขียวบนเขียว")
   *   แต่สร้างความผิดแบบที่สองแทน: ชั้นที่ a = 0.45 ถูกทิ้งทั้งที่เปลี่ยนสีพื้นไปมาก
   *   ⇒ ทางที่ถูกคือทำสิ่งที่เบราว์เซอร์ทำ: **ผสมชั้น** ไม่ใช่เลือกข้าง
   *   วัดจริง: เดิมรายงาน 80 คลาส · ผสมชั้นแล้วรายงาน 113 คลาส และเจอ 5 คลาสที่ contrast < 2.0
   *            (เดิมไม่มีข้อไหนตกลงมาถึงชั้น "มองไม่เห็นเลย" เลยสักข้อ) */
  it('🔴 ตัวตรวจต้องผสมชั้นโปร่งแสงลงบนชั้นทึบ ไม่ใช่ข้ามหรือนับเป็นทึบ', () => {
    expect(audit, 'ไม่มีฟังก์ชันผสมชั้น').toMatch(/overlay\s*=\s*\(/);
    expect(audit, 'ไม่ได้ไล่ผสมจากชั้นล่างขึ้นบน').toMatch(/layers\.length - 1/);
    // ต้องหยุดสะสมเมื่อเจอชั้นทึบ ไม่งั้นไล่ขึ้นไปทั้ง DOM โดยเปล่าประโยชน์
    expect(audit).toMatch(/c\.a >= 1/);
    // และต้องไม่กลับไปใช้เกณฑ์ "ข้ามชั้นโปร่ง" อีก
    expect(audit, 'กลับไปใช้เกณฑ์เดิมที่ข้ามชั้นโปร่งแสง').not.toMatch(/c\.a > 0\.5/);
  });

  it('🔴 ต้องมีชั้น "มองไม่เห็นเลย" แยกออกมา — ของที่อ่านไม่ออกจริงต้องไม่ปนกับของที่แค่จาง', () => {
    expect(audit).toMatch(/มองไม่เห็นเลย/);
  });
});
