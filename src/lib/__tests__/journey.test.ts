...
describe('เมื่อ user ลงมือจริง (ค่าต่างจาก seed) → step ติ๊ก done', () => {
  ...
  it('MIT 24 Steps ระยะ 1 (ขั้น 1–6) ติ๊กครบ → step "รู้จักลูกค้า" done', () => {
    const de24 = DEFAULT_DATA.businessModel.de24.map((s, i) => i <= 5 ? { ...s, done: true } : s);
    const d = { ...DEFAULT_DATA, businessModel: { ...DEFAULT_DATA.businessModel, de24 } };
    expect(byId('mit_customer').done(d)).toBe(true);
    // ยังไม่ครบระยะอื่น
    expect(byId('mit_value').done(d)).toBe(false);
  });
});
