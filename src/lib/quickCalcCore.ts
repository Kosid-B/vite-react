/* quickCalcCore — แกนคำนวณ "ตรวจสินค้าเร็ว" ที่บริสุทธิ์ 100% (ไม่แตะ network/storage)
 *
 * ทำไมต้องแยกไฟล์ (19 ส.ค. 2569): Cloudflare Worker ต้องใช้สูตรชุดนี้เพื่อเรนเดอร์
 *   หน้าเครื่องคำนวณฝั่ง server (เปิดปุ๊บกรอกได้เลย ไม่ต้องรอ JS โหลด)
 *   แต่ productQuickCheck.ts import `supabase` ไว้ ⇒ ลากไลบรารีทั้งก้อนเข้า Worker
 *
 * ⚠️ ห้ามคัดลอกสูตรไปเขียนใหม่ฝั่ง Worker เด็ดขาด — สองชุดจะเพี้ยนจากกันวันใดวันหนึ่ง
 *    แล้วหน้าเว็บกับแอปจะให้คำตอบคนละอย่างจากตัวเลขเดียวกัน (ผู้ใช้เลิกเชื่อทันที)
 *    ไฟล์นี้คือแหล่งความจริงเดียว · productQuickCheck.ts re-export ต่อให้ของเดิมไม่พัง
 */
...
/** ประเภทธุรกิจ (dropdown) — นิยามอยู่ที่นี่เพราะเป็นไฟล์ที่ "ไม่ import อะไรที่มี side effect" เลย
 *  ⚠️ เดิมอยู่ใน skillCatalog.ts ซึ่ง import access.ts → supabase.ts
 *     ทำให้ Cloudflare Worker ที่แค่ต้องการชนิดข้อมูลนี้ ต้องลากไลบรารีทั้งก้อนตามไปด้วย
 *  skillCatalog.ts re-export ต่อ → ไฟล์เดิมทุกไฟล์ยังใช้เหมือนเดิม */
